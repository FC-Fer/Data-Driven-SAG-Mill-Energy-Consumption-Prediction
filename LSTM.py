...
# Functions
#iniciotiempo=time.time() #eliminar
...
def RNN(x, weights, biases, N_lag, N_units):
    ...
    #rnn_cell = rnn.Mu58///////////////////////0''''''''''''''''''''''''''''pltiRNNCell([tf.nn.rnn_cell.DropoutWrapper([rnn.BasicLSTMCell(num_units=n) for n in N_units], output_keep_prob=0.5)])    
...
def Train_SolarRecurrentModel(IndVar, Target, N_lag, n_hidden, training_iters, NumRecurrent, LocModel):
    ...
    # Importing Data Base
    DB_SolarMining = np.loadtxt("Set3norm.txt")
    DB_Testing = np.loadtxt("Set9norm.txt")
