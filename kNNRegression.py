...
def kNN_regression(kvalue, IndVar, Target, Stats):
    ...
    # Importing Data Base
    DB_Training = np.loadtxt("Trainv2.txt")  #Modify (Train data normalized)
    DB_Testing = np.loadtxt("Validationv2.txt") #Modify (Validation data normalized)
    ...
    aVal = 19114 #Modify (target average in train data)
    bVal = int(Stats)
    ...
    np.savetxt('Models_1_3_6_9_10/Sim_Res_Validation_%s_%s.txt'%(kvalue,Target), Results, fmt='%3.4e', delimiter='\t', newline='\n')      
...
#k_value = [1,2,3,4,5]
k_value =  np.linspace(1,50,50) #Modify if necessary
TargetVec = [14] #Modify (target columns)

for ind_0 in range(len(TargetVec)):  
    for ind_1 in range(len(k_value)):
        #IndVar = [1, 2, 3, 4, 7, 8]
        #IndVar = [1, 2, 3, 4]   
        IndVar = [1,3, 6, 9, 10]   #Modify (Input columns)      
